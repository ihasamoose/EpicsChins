...
public class Banking extends Strategy implements Runnable {
	...
	public void run() {
		...
		if (Inventory.getCount() == 28 && Inventory.getCount(GUI.foodUser) < 1) {
			Context.get().getActiveScript().log.info("Supplies are done wrong, restarting...");
	...
	public boolean validate() {
		...
		return (Inventory.getCount(Data.POT_PRAYER) <= 1 && Data.START_SCRIPT && Game.isLoggedIn() && !Data.runCheck || Data.chinNumber <= 100 && Data.START_SCRIPT && Game.isLoggedIn() && !Data.runCheck || Method.isPoisoned() && antipoisonData == 0 && Data.START_SCRIPT && Game.isLoggedIn() && !Data.runCheck || Players.getLocal().getHpPercent() <= 25) && !Data.runCheck && Data.START_SCRIPT && Game.isLoggedIn() || Inventory.getCount(Data.TAB_VARROCK) == 0 && Data.START_SCRIPT && Game.isLoggedIn() && !Data.runCheck;
	}
}
